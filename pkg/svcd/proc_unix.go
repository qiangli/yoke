...
// The unix half of the daemon lifecycle (see svcd.go). One property is worth
// keeping in view: signals go to the daemon's process GROUP only when the pid
// actually leads one, so a stale or recycled pidfile can never take down an
// unrelated group.
